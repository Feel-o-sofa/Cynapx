...
2. [설정](#2-설정)
   ...
   - [2.6 모델 비종속 임베딩](#26-모델-비종속-임베딩)
3. [프로젝트 라이프사이클](#3-프로젝트-라이프사이클)
4. [MCP 도구 레퍼런스](#4-mcp-도구-레퍼런스)
   - [4.1 설정 및 라이프사이클](#41-설정-및-라이프사이클)
   - [4.2 심볼 탐색 및 시맨틱 검색](#42-심볼-탐색-및-시맨틱-검색)
   - [4.3 시간적 컨텍스트 및 에이전트 메모리](#43-시간적-컨텍스트-및-에이전트-메모리)
   - [4.4 아키텍처 분석](#44-아키텍처-분석)
   - [4.5 품질 및 위험도](#45-품질-및-위험도)
   - [4.6 리팩토링 및 내보내기](#46-리팩토링-및-내보내기)
...
└──────────────────────────┬───────────────────────────┘
       ...
       (stdio)         (:3000)    (BFS/DFS/LRU 캐시)
       27개 도구     Swagger UI     영향 분석 순회
```
...
`.mcp.json` 저장 후 Claude Code를 재시작합니다. Cynapx가 MCP 도구 패널에 나타나고 27개 도구가 모두 사용 가능해집니다.
...
| 옵션 | 기본값 | 설명 |
|------|--------|------|
| `-p, --path <paths...>` | `cwd` | 분석할 프로젝트 경로 (공백으로 구분, 여러 개 가능) |
| `--api` | 꺼짐 | REST API 서버 시작 (기본은 꺼짐 — MCP stdio 전용) |
| `--api-port <number>` | `3000` | REST API 및 Swagger UI 포트 |
| `--bind <address>` | `127.0.0.1` | 바인드 주소. LAN 접근을 위해 `0.0.0.0` 사용 |
| `--https` | 꺼짐 | API 서버에 임시 자체 서명 HTTPS 인증서 활성화 |
| `--no-index` | 인덱싱 켜짐 | 시작 시 초기 인덱스 동기화 건너뛰기 |
| `--no-watch` | 감시 켜짐 | 파일 워처 비활성화 (저장 시 증분 재인덱싱 안 함) |
| `--force` | 꺼짐 | 증분 대신 전체 재인덱싱 강제 |
| `--interactive` | 꺼짐 | MCP stdio 대신 대화형 REPL 모드로 시작 |

**인증 (REST 전용):** API Bearer 토큰은 `KNOWLEDGE_TOOL_TOKEN` 환경변수에서 읽습니다. 이 변수가 설정되지 않으면 토큰이 자동 생성되어 `~/.cynapx/api-token` 파일(모드 `0600`)에 기록됩니다 — stdout/stderr로는 출력되지 **않습니다**. REST 요청 시 `Authorization: Bearer <token>`으로 전송합니다. MCP 서버(stdio)는 토큰을 사용하지 않습니다(REST 전용). `GET /healthz`는 항상 인증 없이 접근 가능합니다.

**환경변수:** `CYNAPX_LOG_LEVEL`(`debug`\|`info`\|`warn`\|`error`\|`silent`)로 로그 상세도 조정(로그는 stderr로 출력); `CYNAPX_LOG_PAYLOADS=1`로 마스킹된 요청 페이로드 로깅 활성화; `NODE_ENV=production`이면 `/api/docs`의 Swagger UI 비활성화. 임베딩 제공자 변수는 [§2.6](#26-모델-비종속-임베딩) 참고.

---

### 2.6 모델 비종속 임베딩

Cynapx의 시맨틱 계층(벡터 검색, `find_similar_symbols`, `search_symbols`의 선택적 `semantic: true` 모드)은 **플러그형**입니다. 임베딩 제공자는 런타임에 선택되므로 Cynapx는 벤더 종속 없이 어떤 AI 에이전트 모델이든 — Claude, ChatGPT Codex, 로컬 LLM, 미래 모델 — 지원할 수 있습니다. 제공자를 설정하지 않으면 번들된 Jina code-embeddings 사이드카가 사용됩니다.
...
| 제공자 | 전송 방식 | 기본 모델 | 비고 |
|--------|-----------|-----------|------|
| `openai` | 네이티브 `fetch` → `/v1/embeddings` | `text-embedding-3-small` (1536) | OpenAI, Azure OpenAI, vLLM, LM Studio 호환 |
| `ollama` | 네이티브 `fetch` → `localhost:11434/api/embed` | `nomic-embed-text` (768) | 완전 로컬 |
| `jina-sidecar` | Python 사이드카 | `jina-code-embeddings` (896) | 번들 기본값 |
| `null` | — | — | no-op 폴백; 시맨틱 기능 비활성화 |

**프로젝트별 오버라이드.** 프로젝트 프로파일에 `embedding` 블록을 설정하여 환경 기본값을 덮어쓰고 특정 프로젝트의 제공자를 고정할 수 있습니다.

**쿼리 타임 패스스루.** 자체 모델로 이미 임베딩을 계산한 에이전트는 사전 계산된 `query_embedding` 배열을 `search_symbols`에 전달할 수 있습니다. 서버는 재임베딩을 생략하고 제공된 벡터로 직접 검색하여 — 쿼리를 에이전트 자신의 모델 공간에 유지하고 모델 간 불일치를 방지합니다. 벡터를 제공하면 `semantic: true` 없이도 시맨틱 검색이 활성화됩니다.

> **제공자 변경 후 재인덱싱.** 저장된 임베딩은 인덱스 시점에 활성화된 제공자가 생성합니다. 제공자(또는 차원)를 변경하면 `purge_index` 후 `initialize_project`를 실행하여 임베딩을 일관되게 재생성하세요.
...
토큰 효율적인 프로젝트 전체 브리핑을 반환합니다 — 에이전트가 낯선 코드베이스에서 컨텍스트를 가장 빠르게 부트스트랩하는 방법입니다. 목적, 기술 스택, 아키텍처 형태, 엔트리 포인트, 핫스팟, 문서 헤더를 요약합니다.

**파라미터:**

| 이름 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| `include_clusters` | boolean | `true` | 브리핑에 아키텍처 클러스터 분해 포함 여부 |

**반환값:** 목적, 기술 스택, 아키텍처 형태, 엔트리 포인트, 핫스팟, 문서 헤더를 담은 텍스트 브리핑.
...
이름이나 설명으로 지식 그래프의 심볼을 검색합니다. 기본적으로 정확한 접두사 매칭을 지원하며, 임베딩 제공자가 설정된 경우 벡터 유사도 검색, 그리고 모델 비종속 쿼리를 위한 **쿼리 타임 벡터 패스스루**도 가능합니다.
...
| `semantic` | boolean | `false` | 벡터 유사도 검색 활성화 (임베딩 제공자 설정 필요) |
| `query_embedding` | number[] | — | 사전 계산된 쿼리 벡터. 제공 시 서버는 재임베딩을 생략하고 이 벡터로 직접 검색 — `semantic: true` 없이도 시맨틱 검색 활성화. [2.6](#26-모델-비종속-임베딩) 참고 |

**반환값:** `{ qname, type, file, signature, docstring_snippet, tags, fan_in, score }` 배열. `score` 필드는 하이브리드(RRF) 또는 위치 기반 관련도를 노출하여 에이전트가 순위·필터링할 수 있게 합니다.
...
저장된 임베딩에 대한 K-최근접 이웃(K-NN) 검색으로 주어진 심볼과 **의미적으로 유사한** 심볼을 찾습니다. 중복 탐지, 반복 패턴 발굴, 리팩토링 후보 발견에 유용합니다. 쿼리 심볼 자기 자신은 결과에서 제외됩니다.

**파라미터:**

| 이름 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| `qualified_name` | string | 필수 | 이웃을 찾을 심볼 — 이미 인덱싱·임베딩되어 있어야 함 |
| `limit` | number | `10` | 반환할 최대 유사 심볼 수 |

**반환값:** `{ qname, type, file, signature, docstring_snippet, tags, fan_in, score }` 배열, 유사도 내림차순 정렬. `score`는 `1 / (1 + distance)`로 정규화됩니다(1.0 = 동일, 거리가 커질수록 → 0).

> 임베딩 제공자 설정과 인덱스 시점에 생성된 임베딩이 필요합니다. 심볼에 저장된 임베딩이 없으면 빈 결과가 반환됩니다.

**호출 예시:**

```
도구: find_similar_symbols
...
### 4.3 시간적 컨텍스트 및 에이전트 메모리

이 도구들은 *"최근 무엇이 바뀌었나"* 와 *"왜 이것이 존재하나"* 에 답하고, 에이전트가 어렵게 얻은 컨텍스트를 지식 그래프에 영속적으로 기록하여 이후 세션이 이를 물려받도록 합니다.

---

#### `get_recent_changes`

코드베이스 전반의 최근 커밋과 각 커밋이 변경한 심볼/파일을 나열합니다 — *"최근 무엇이 바뀌었나?"* 에 답합니다. 세션 시작 시 방향을 잡거나 리뷰 범위를 설정할 때 사용합니다.

> **`backfill_history`를 먼저 실행**해야 합니다 (Git 커밋을 심볼에 매핑).

**파라미터:**

| 이름 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| `limit` | number | `20` | 반환할 최대 커밋 수 |
| `since_days` | number | — | 최근 N일 이내의 커밋만 포함 |

**반환값:** 각 커밋과 해당 커밋이 건드린 심볼/파일, 최신순.
...
단일 심볼의 전체 커밋 이력을 의도 요약과 함께 반환합니다 — 해당 심볼을 건드린 모든 커밋을 순서대로. *"왜 이것이 존재하나 / 왜 이런 형태인가?"* 에 답합니다.

> **`backfill_history`를 먼저 실행**해야 합니다.

**파라미터:**

| 이름 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| `qualified_name` | string | 필수 | 이력을 조회할 심볼 |

**반환값:** 심볼의 커밋 이력(commit, date, author, message)과 의도 요약, 최신순.
...
에이전트가 작성한 노트를 심볼에 영속화합니다. 어노테이션은 세션과 모델을 넘어 유지됩니다 — 지식 베이스가 이를 사용하는 에이전트로부터 *학습*하는 방식입니다.

**파라미터:**

| 이름 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| `qualified_name` | string | 필수 | 어노테이션을 달 심볼 |
| `kind` | `"decision"` \| `"gotcha"` \| `"todo"` \| `"rationale"` | 필수 | 노트의 범주 |
| `body` | string | 필수 | 어노테이션 본문 |
| `author` | string | `"agent"` | 노트 작성자 |

**반환값:** 저장된 어노테이션 ID를 포함한 확인 메시지.
...
qualified_name: "UserService.authenticate"
kind: "gotcha"
body: "토큰 TTL은 호출 시점에 env에서 읽으며 캐시되지 않음 — AUTH_TTL 변경은 즉시 반영됨."
```
...
심볼에 대한 에이전트 어노테이션을 조회합니다 — 심볼을 지정하지 않으면 코드베이스 전반의 최근 어노테이션을 반환합니다.

**파라미터:**

| 이름 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| `qualified_name` | string | — | 어노테이션을 조회할 심볼. 생략 시 코드베이스 전반의 최근 어노테이션 나열 |
| `kind` | `"decision"` \| `"gotcha"` \| `"todo"` \| `"rationale"` | — | 어노테이션 범주로 선택적 필터 |
| `limit` | number | `20` | 반환할 최대 어노테이션 수 |

**반환값:** `{ id, kind, body, author, created_at }` 배열.
...
[gotcha]    토큰 TTL은 호출 시점에 env에서 읽으며 캐시되지 않음.
[decision]  CLI 클라이언트까지 커버하기 위해 컨트롤러가 아닌 여기서 MFA를 강제함.
```
...
프로젝트의 **선언된 아키텍처 의도**(`cynapx.architecture.json`)와, 이를 실제 그래프와 대조하는 **드리프트 리포트**를 함께 보고합니다 — 어떤 의도된 레이어/의존성이 존재하는지, 그리고 어떤 실제 엣지가 선언된 설계를 위반하는지.

**파라미터:** 없음

**반환값:** 선언된 레이어/규칙, 충족된 의도, 탐지된 드리프트를 담은 텍스트.

**출력 예시:**

```
Declared architecture (cynapx.architecture.json):
  Layers: api → service → db   (하향만 허용)

Drift detected (1):
  ✗ db/Database.ts imports service/UserService.ts  (상향 의존성)
```
