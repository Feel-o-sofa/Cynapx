...
토큰 효율적인 프로젝트 전체 브리핑을 반환합니다 — 에이전트가 낯선 코드베이스에서 컨텍스트를 가장 빠르게 부트스트랩하는 방법입니다. 전체 그래프를 덤프하지 않고 규모, 언어 구성, 가장 바쁜 모듈, `fan_in` 기준 상위 심볼을 요약합니다.

**파라미터:**

| 이름 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| `top_n` | number | `10` | 각 순위 목록에 포함할 상위 심볼/모듈 수 |

**반환값:** 노드/엣지 총계, 언어 구성, 엔트리 포인트, 가장 많이 의존되는 심볼을 담은 텍스트 브리핑.
...
그래프에 매핑된 Git 이력에서 도출된 최근 변경 심볼을 나열합니다. 세션 시작 시 방향을 잡거나 리뷰 범위를 설정할 때 사용합니다.

**파라미터:**

| 이름 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| `limit` | number | `20` | 반환할 최대 변경 심볼 수 |
| `since` | string | — | 선택적 ISO 날짜 또는 Git 리비전 하한 (예: `2026-06-01`, `HEAD~20`) |

**반환값:** `{ qname, file, last_commit, commit_date, message }` 배열, 최신순.
...
단일 심볼의 변경 이력과 존재 이유를 반환합니다 — 해당 심볼을 건드린 모든 커밋을 순서대로. *"왜 이것이 존재하나 / 왜 이런 형태인가?"* 에 답합니다.
...
**반환값:** `{ commit, date, author, message }` 배열, 최신순.
...
qualified_name: "UserService.authenticate"
kind: "gotcha"
text: "토큰 TTL은 호출 시점에 env에서 읽으며 캐시되지 않음 — AUTH_TTL 변경은 즉시 반영됨."
```
...
심볼에 대한 에이전트 작성 어노테이션을 모두 조회합니다.

**파라미터:**

| 이름 | 타입 | 설명 |
|------|------|------|
| `qualified_name` | string | 어노테이션을 조회할 심볼 |

**반환값:** `{ id, kind, text, created_at }` 배열.
