...
- **architecture-engine.ts:179 `edges.find` O(E) → O(1) Map** → **actionable(M-1 v19로 승격)**. 두 사실이 결정적: (i) `edges`가 이미 line 87에서 fetch돼 있어 Map 한 번 구축이 ~5줄·시그니처 무변경, (ii) 이 `edges.find`가 사는 circular-dependency 분기(165-185)가 *현 vitest에 미커버*(stub `getOutgoingEdges: () => []`라 `detectCycles()`가 cycle 0개) → 최적화가 곧 게이트 격차 메우기. → Phase 18~20 패턴과 동형.
