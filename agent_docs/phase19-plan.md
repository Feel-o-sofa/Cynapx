...
## 2. Phase 19-1: 8개 REST 핸들러 동작 분기 supertest 테스트 추가 (M-1 v16) — 테스트-only·무위험 [DONE]

> **[DONE — Phase 19-1]** `tests/api-server-http.test.ts`에 신규 supertest 케이스 **10건** 추가(테스트-only, `src/` 무변경). 커버: `handleGetSymbol` 404 SYMBOL_NOT_FOUND + 200(`node.symbol.qualified_name`), callers/callees/impact/tests 404 가드 4건(impact/tests는 공유 analyzeLimiter 싱글톤 예산 소진 시 429 허용으로 결정성 유지), callers 200(빈 traverse), `handleExportGraph` 200(`{format:'mermaid', content}`) + 500 EXPORT_FAILED, `validate()` 400 Validation failed(`/api/search/symbols` 빈 query). 검증: `npx vitest run` **588/588** 그린, `npx tsc --noEmit` 그린, `npm audit --omit=dev` 0 vulnerabilities.
...
| 19-1 **[DONE]** | M-1 v16: REST 핸들러 동작 분기 supertest 테스트 **10건** 추가(handleGetSymbol 404/200 우선 + callers/callees/impact/tests 404 가드 + callers 200 + export 200/500 + validate 400) + diagnostic-v16 [DONE] — **588/588** 그린, tsc 그린, audit 0 vulns | 1 | 매우 낮음 (테스트-only, 프로덕션 코드 무변경) |
