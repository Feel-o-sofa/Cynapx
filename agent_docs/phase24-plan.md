...
## 2. Phase 24-1: PolicyDiscoverer 로직 게이트 + 핸들러 인자 검증 정렬 (M-1 v21 + M-2 v21) [예정]
...
## 3. Phase 24-2 (선택): 문서 Node 버전 드리프트 정렬 (L-12) — docs-only [예정]
...
| 24-1 [예정] | M-1 v21: `tests/policy-discoverer.test.ts` 신규(테스트-only) + M-2 v21: `discover-latent-policies.ts` 인자명 정렬(~5줄) + 디스패처 게이트 — 같은 도구·같은 파일 | 1 | 매우 낮음 |
| 24-2 (선택) [예정] | L-12: README_KR/GUIDE_EN/GUIDE_KR Node "≥ 20" → "≥ 22" 정렬 | 1 (24-1과 합본 가능) | 없음 (docs-only) |
