...
## 2. Phase 25-1: RefactoringEngine.proposeRefactor() 로직 게이트 (M-1 v22) [DONE]
...
## 3. Phase 25-2: search-symbols.ts query 인자 검증 추가 (M-2 v22) [DONE]
...
| 25-1 [DONE] | M-1 v22: `tests/refactoring-engine.test.ts`에 `proposeRefactor()` 게이트(동일 파일 하니스 재사용, 테스트-only) — risk 4-티어·reasons 6-분기·steps risk-별·null-노드 (신규 12 케이스, vitest 634/634) | 1 | 매우 낮음 |
| 25-2 [DONE] | M-2 v22: `search-symbols.ts` `query` string-required 가드(~3줄, 형제 핸들러 동형) + 디스패처 케이스 (신규 4 케이스, vitest 634/634) | 1 (25-1과 합본 가능) | 매우 낮음 |
