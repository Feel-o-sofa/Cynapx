...
1. **P28-1 [예정]**: M-1 v25 해소 — `tests/phase12-6-commit-b.test.ts`(O-2 describe 옆)에 `toCanonical` 변환-동작 케이스 추가(역슬래시→슬래시·빈-문자열·drive-letter·상대→절대 prepend(+`//` 가드)·소문자·다중-슬래시 축약·trailing-slash 제거, 의존 0, 테스트-only, prod 코드 무변경). (vitest 649→656~ 그린)
2. **P28-2 [예정]**: M-2 v25 해소 — `npm i express@4.22.2`로 `package-lock.json` express 엔트리만 4.22.1→4.22.2 정렬(`package.json` 핀 `^4.19.2` 무변경) + 649 그린(api-server supertest 회귀 게이트) + audit 0/0 재확인. **lockfile-only.** (긴급도 낮음 — 가벼운 사이클을 원하면 L-16 추적으로 deferral 가능)
