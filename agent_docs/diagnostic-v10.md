...
### C-1. Docker 배포 경로 전손 — 런타임 이미지에 schema/·scripts/ 미포함, prepare 스크립트가 빌드 자체를 깨뜨림 **[DONE — Phase 13-1]**
**`Dockerfile:29-37, 50-54`, `src/db/database.ts:56-65`, `package.json:18`**

> **[DONE — Phase 13-1]** (1) 런타임 스테이지 `COPY schema/` + 빌더에서 prune한 node_modules 통째 복사(런타임 npm ci 제거), `.dockerignore` 신설 — 단 `scripts/cynapx_embedder.py`는 리포에 실존하지 않는 파일이라 미포함(FTS5 폴백으로 강등, Dockerfile에 문서화). (2) `prepare`→`prepack`. (3) `resolvePythonCommand()`로 python3→python 폴백(둘 다 없으면 spawn 없이 폴백 모드). Node 22 베이스, USER node, `/healthz` 인증 면제+pending 503, `scripts/docker-smoke.sh` 신규. 샌드박스 Docker 데몬에서 빌드+기동+healthz 200 스모크 PASS — 단 apt 단계는 샌드박스 egress 차단으로 동등 변형(툴체인 포함 베이스)으로 대체 검증(상세: phase13-plan.md 2장 [DONE] 노트).
...
### C-2. 임베딩 사이드카 spawn 'error' 미처리 → `python` 미설치 환경에서 서버 전체 크래시 **[DONE — Phase 13-2]**
**`src/indexer/embedding-manager.ts:55-67`, `src/bootstrap.ts:40-45`**

> **[DONE — Phase 13-2]** `child.on('error', ...)`를 exit 핸들러와 공유 실패 경로(`handleChildGone`, 멱등 가드)로 추가 — spawn ENOENT/EACCES가 더 이상 uncaughtException으로 호스트를 죽이지 않고 재시도/FTS5 폴백으로 강등. `scriptPath`를 `process.cwd()` → `path.resolve(__dirname, '..','..','scripts',...)` 패키지 루트 기준으로 교체. 유닛 5건 추가(error→restart, error+exit 멱등, 재시도 소진→폴백, dispose 후 error 무시, 패키지 루트 경로).
...
### C-3. IPC "챌린지-응답"이 시크릿 nonce를 평문으로 먼저 송신 — 인증 사실상 무력화 **[DONE — Phase 13-2]**
**`src/server/ipc-coordinator.ts:69-90`**

> **[DONE — Phase 13-2]** Host는 이제 nonce와 무관한 일회용 랜덤 challenge(32B)를 송신, Terminal은 `HMAC-SHA256(key=nonce, msg=challenge)`로 응답, Host는 `timingSafeEqual`로 검증 — nonce는 어떤 방향으로도 와이어에 실리지 않음(`computeAuthResponse` 헬퍼 추출). 회귀 테스트: challenge를 에코하는 클라이언트·잘못된 nonce HMAC 모두 인증 거부(소켓 destroy, executeTool 미호출), 올바른 HMAC만 성공, 연결당 challenge 유일성, Terminal write에 nonce 부재 검증. **pre-fix 코드 대비 검증**: git stash로 구 코드에서 동일 에코-공격 테스트가 즉시 실패함(challenge === nonce)을 확인.
...
### H-1. PID 재사용·죽은 IPC 포트 시 Terminal이 무한 재시도 — heartbeat는 쓰기만 하고 아무도 읽지 않음 **[DONE — Phase 13-3]**
**`src/utils/lock-manager.ts:57-61, 187-191`, `src/bootstrap.ts:239-244, 265`**

> **[DONE — Phase 13-3]** `isHeartbeatStale()`(90s `HEARTBEAT_STALE_MS`) + 순수 `decideConnectFailureAction(stale, retries, max=5)`로 connect 재시도 상한 도입 — stale이거나 5회 도달 시 `forceReclaim(nonce)` 후 재acquire 에스컬레이션(무한 2s 루프 제거). heartbeat 타이머를 `startHeartbeatTimer()`로 통합해 `attemptFailover` 승격 경로에도 가동.
...
### H-2. heartbeat()의 비원자적 in-place 덮어쓰기 → 경합 reader가 락을 "corrupt"로 오판·삭제 → split-brain **[DONE — Phase 13-3]**
**`src/utils/lock-manager.ts:110-113, 187-191, 208-213`**

> **[DONE — Phase 13-3]** `atomicWrite()`(tmp+`renameSync`)로 heartbeat/signalShutdown 원자화 — 경합 reader가 잘린 JSON 미관측. corrupt 삭제 경로는 50ms 후 1회 재독·재liveness 후에만 삭제. `release()`는 on-disk nonce 자기 일치 시에만 unlink(failover 교체 후 타인 락 보호). 100회 인터리브 오판 0회 회귀.
...
### H-3. 증분 동기화: from-커밋 소실(rebase/force-push/shallow) 시 영구·무음 동기화 중단 **[DONE — Phase 13-4]**
**`src/indexer/git-service.ts:60-87`, `src/indexer/sync-strategies/incremental-sync-strategy.ts:17-19`**

> **[DONE — Phase 13-4]** `getDiffFiles()`가 diff 명령 실패 시 `DiffFailedError`를 throw(빈 diff `[]`와 구분), `--name-status -z` NUL 파싱으로 공백 포함 경로 대응. `syncWithGit()`는 `commitExists(lastCommit)` 선검사 + `DiffFailedError` catch로 from-커밋 소실 시 `FullScanStrategy` 폴백 → 워터마크 복구. 실제 임시 git 리포로 orphan/gc 후 폴백·복구·공백파일 회귀(`tests/git-sync.test.ts`).
...
### H-4. 열린 트랜잭션(BEGIN) 안에서 await — 파일당 순차 git subprocess 호출 **[DONE — Phase 13-4]**
**`src/indexer/update-pipeline.ts:273, 285` (`applyDelta`도 동일: 349, 359)**

> **[DONE — Phase 13-4]** `prefetchHistories(filePaths)` 신설(`mapHistoryToProject`의 CHUNK_SIZE=20 청크 병렬 + de-dup) — `processBatch`/`applyDelta`가 BEGIN **이전**에 전체 히스토리를 프리페치하고 트랜잭션 본문은 순수 동기(await 0). O-2: `FullScanStrategy`/`IncrementalSyncStrategy`의 파일당 `getLatestCommit`을 `getLatestCommitsForFiles()`(`git log --name-only` 단일 패스)로 대체. 회귀: BEGIN 이전 프리페치 호출 순서·동일 매핑·de-dup 검증(`tests/update-pipeline-batch.test.ts`).
...
### H-5. 비-TS 12개 언어의 cyclomatic complexity가 사실상 가짜 값 **[DONE — Phase 13-5]**
**`src/indexer/tree-sitter-parser.ts:112`, `src/indexer/metrics-calculator.ts:49-88`, `src-native/src/lib.rs:46-54`**

> **[DONE — Phase 13-5]** `calculateCyclomaticComplexityTreeSitter(node, decisionPoints)` 신설(tree-sitter 트리 직접 순회 → 결정점 노드 카운트, 문자열/주석 내 키워드 미카운트, 논리연산자만 분별 +1). tree-sitter 파서를 이 경로로 일원화, native 토큰 카운터(lib.rs) 삭제, Go `expression_case_clause` 오타 등 디스크립터 보정. Rust/Go/Java/Python 손계산 CC 회귀 12건 추가(388/388).
...
### H-6. purge_index가 좀비 컨텍스트를 남김 — onPurge 미배선, 재초기화 불능, 닫힌 DB 핸들 사용 **[DONE — Phase 13-6]**
...
> **[DONE — Phase 13-6]** `WorkspaceManager.unmountProject(hash)` 신설 — watcher → workerPool → dbManager dispose(12-4 H-5 역순) 후 모든 엔진 필드 null화(컨텍스트 path/hash 보존). bootstrap에서 `mcpServer.setOnPurge(unmountProject)` 최초 배선(+A-11 프로젝트 락 release). `onPurge`를 `(hash)` 인자로 확장. dbManager null화로 `initializeEngine`/`startHostServicesForContext` 가드가 dispose된 핸들을 살아있다 오판하지 않아 purge→initialize_project 재빌드 정상. 테스트: `tests/workspace-manager.test.ts`(6) + 통합 Phase 25(purge→re-init→search).
...
### H-7. 경로 경계 검사 3곳이 separator 없는 prefix-match — sibling 디렉터리 우회 **[DONE — Phase 13-7]**
**`src/utils/security.ts:43`, `src/server/mcp-server.ts:115-119`, `src/utils/paths.ts:179`**

> **[DONE — Phase 13-7]** `isPathInside(child, parent)` 공용 헬퍼 신설(`path.relative` 기반, sibling `<root>-secrets` 차단, win32만 case-insensitive). 3곳 + `initialize-project.ts` 2곳 통일. 단위 매트릭스 테스트 추가.
...
### H-8. IPC 누적 바이트 제한이 정상 장수 연결을 강제 절단 **[DONE — Phase 13-2]**
**`src/server/ipc-coordinator.ts:60-67`**

> **[DONE — Phase 13-2]** 누적 `totalBytes`를 라인(메시지) 단위 `currentLineBytes`로 교체 — 청크 내 `\n` 경계마다 카운터 리셋, 현재 줄 길이만 1MB와 비교. 장수 연결이 누적 1MB+ 정상 트래픽에서도 유지되고, 단일 1MB+ 메시지(개행 유무 무관)는 여전히 절단. 유닛 3건(누적 1.2MB 후 연결 생존+동작, 단일 초과 절단, 개행 동반 초과 절단).
...
### H-9. `--https` 실패 시 무음 평문 HTTP 폴백 **[DONE — Phase 13-2]**
**`src/bootstrap.ts:306-313`**

> **[DONE — Phase 13-2]** `src/utils/https-options.ts`(`resolveHttpsOptions`) 신설 — `--https` 요청 시 인증서 생성 실패는 `HttpsUnavailableError` throw → bootstrap이 `process.exit(1)`(무음 평문 폴백 제거). 비-루프백 `--bind` + 평문 HTTP 조합은 경고 출력(`isLoopbackAddress` 헬퍼). 유닛: 생성 실패 시 throw, 성공 시 TLS 반환, 비-루프백 평문 경고, 루프백 무경고, isLoopbackAddress 매트릭스.
...
### A-1. `INSERT OR REPLACE` + `recursive_triggers` OFF → FTS 고아 행 누적, cross-file 에지 소실 **[DONE — Phase 13-4]**
**`src/db/node-repository.ts:34-49`, `schema/schema.sql:94-105`, `src/db/database.ts:36-47`, `src/indexer/update-pipeline.ts:342-374`**

> **[DONE — Phase 13-4]** `database.ts`에 `PRAGMA recursive_triggers = ON` 추가. `createNode()`를 `INSERT ... ON CONFLICT(qualified_name) DO UPDATE ... RETURNING id`로 교체 — 노드 id 보존(타 파일발 에지 소실 차단) + `nodes_au` AFTER UPDATE 트리거가 fts_symbols 동기화(고아 행 0). UPDATE 분기에서 stale `node_tags` 선삭제 후 재삽입. 회귀: id 보존·cross-file 에지 유지·재인덱스 5회 후 fts 1행·pragma ON(`tests/database-migration.test.ts`).
...
### A-2. API MCP 세션 맵 무한 증가 + sessionId가 로그에 노출되는 인증 토큰 역할 **[DONE — Phase 13-7]**
**`src/server/api-server.ts:120, 153-169, 171-186, 245-271`**

> **[DONE — Phase 13-7]** idle TTL 30분(5분 sweep + 요청별 lazy sweep) + 상한 100(초과 시 429 거부) 도입. 요청 로거에서 `sessionId` 쿼리 파라미터 마스킹(prefix 8자 + `***`). HTTP 레벨 테스트 추가.
...
### A-3. Bearer 토큰 비교가 non-constant-time **[DONE — Phase 13-7]**
**`src/server/api-server.ts:182`**

> **[DONE — Phase 13-7]** `timingSafeEqualStr` — 양측 SHA-256(32바이트 고정) 해시 후 `crypto.timingSafeEqual` 비교. 길이 불일치 throw 회피 + 길이 기반 조기 종료 타이밍 누출 제거.

`authHeader !== \`Bearer ${AUTH_TOKEN}\`` — `crypto.timingSafeEqual`로 교체(길이 불일치 처리 포함). 로컬호스트 기본값에선 위험도가 낮지만 `--bind 0.0.0.0` 운용 시 의미가 생긴다.

### A-4. 배치마다 전 노드 fan 메트릭 풀 재계산 + 미해석 에지당 leading-wildcard LIKE 풀스캔 — **[DONE — Phase 13-8]**
**`src/indexer/update-pipeline.ts:435-441`, `src/db/node-repository.ts:320-325`**

> **[DONE — Phase 13-8 커밋 A]** `recomputeFanMetrics(changedNodeIds?)`를 배치 변경 노드 집합으로 한정. `symbol_name` 컬럼 + **COLLATE NOCASE 인덱스**(마이그레이션 2→3, 백필 포함) 추가, `findNodesBySymbolName()`을 단일 indexed equality probe로 교체(글로벌 심볼은 symbol_name == qualified_name이라 OR 불필요). 검증 중 발견·수정: NOCASE 쿼리가 BINARY 인덱스를 못 쓰므로 인덱스 자체를 NOCASE로 선언해야 SEARCH가 됨.
...
### A-5. NodeRepository가 호출마다 prepare / persistClusters 루프가 트랜잭션 밖 — **[DONE — Phase 13-8]**
**`src/db/node-repository.ts:35, 86, 101-108`, `src/graph/graph-engine.ts:247-303`**

> **[DONE — Phase 13-8 커밋 A]** NodeRepository에 prepared-statement 캐싱(`_upsertStmt` 등 + `invalidateStatementCache()`, WorkspaceManager onMigration에 배선). `persistClusters()` 전체(reset + 클러스터 INSERT + `updateCluster()` 루프)를 단일 트랜잭션으로 — 중간 실패 시 전무(원자성 테스트 추가).
...
### A-6. 워처 경로가 gitignore를 무시 + ProjectProfile이 통째로 죽은 설정 — **[DONE — Phase 13-8]**
**`src/watcher/file-watcher.ts:48-52`, `src/utils/file-filter.ts`, `src/utils/profile.ts:14-35`, `src/server/workspace-manager.ts:88`**

> **[DONE — Phase 13-8 커밋 B]** `FileFilter`를 chokidar `ignored` 콜백(디렉터리 prune 포함)과 `handleChange()`에 배선 — 워처가 ConsistencyChecker와 동일한 .gitignore 기준으로 무시. `excludePatterns`/`maxFileSize`를 `FileFilter`(신규 `FileFilterOptions`, `shouldIgnoreFile()`)에 실효 배선하고 워처(`bootstrap`에서 `ctx.profile` 전달)와 ConsistencyChecker 양쪽에 적용 → 죽은 설정 해소. (`languageOverrides`/`webhookUrl`은 본 항목 범위 밖, 그대로 유지.)
...
### A-7. 임베딩 IPC에 요청-응답 상관관계 없음 → 타임아웃 후 응답이 다음 배치에 오배달 — **[DONE — Phase 13-8]**
**`src/indexer/embedding-manager.ts:77-89, 152-156, 251-267`**

> **[DONE — Phase 13-8 커밋 A]** 요청에 단조 증가 `id` 추가, 응답 id 불일치 시 폐기, 응답 직전 잔존 pendingRequest는 supersede로 reject. 응답 핸들러를 `handleSidecarMessage()`로 추출해 mock 없이 회귀 테스트(늦은 응답 오배달 차단, id 미부착 응답 하위호환).
...
### A-8. 프로젝트 해시가 lowercase MD5 — 케이스 구분 FS에서 서로 다른 프로젝트가 같은 DB/락 공유 — **[DONE — Phase 13-8]**
**`src/utils/paths.ts:193-196`, (관련: 98-131 레지스트리 lost-update)**

> **[DONE — Phase 13-8 커밋 B]** `getProjectHash()`의 lowercase를 win32/darwin에만 적용(`isCaseInsensitiveFs()`, H-7 `isPathInside`와 동일 규칙) → Linux에서 케이스 다른 경로는 별개 DB/락. 레지스트리 3개 write(`addToRegistry`/`removeFromRegistry`/`updateRegistryStats`)를 `updateRegistryAtomic()`로 통합 — pid+random tmp 파일명 + 충돌 시 재read 후 재시도(lost-update 완화). `saveProfile`도 pid 기반 tmp로.
...
### A-9. admin CLI가 라이브 DB에 비안전 백업/VACUUM/복원 **[DONE — Phase 13-6]**
**`src/cli/admin.ts:300-311, 351-372, 393-396, 449-451`**

> **[DONE — Phase 13-6]** `backup`을 `VACUUM INTO ?` 온라인 백업으로 교체(`onlineBackup()` — 라이브 WAL DB에서도 단일 일관 스냅샷, sidecar 없음). `purge`/`compact`/`restore` 실행 전 `assertNoLiveHost()`가 신규 read-only `LockManager.probeProjectLock()`(Phase 13-3의 PID 생존+heartbeat staleness 재사용, side-effect 0)로 라이브 Host 검출 → 존재 시 거부, `--force`로만 우회. 테스트: `tests/admin-cli.test.ts`(7 — probeProjectLock 매트릭스 + VACUUM INTO integrity_check).
...
| 항목 | 위치 | 상태 |
|------|------|------|
| package.json 버전 읽기 5곳 중복 (v9 A-6) | `bootstrap.ts:48`, `mcp-server.ts:52-55,161-165`, `workspace-manager.ts:114,180`, `admin.ts:460`, `api-server.ts:207-212` | **[DONE — Phase 13-8]** `utils/version.ts`(`getVersion()`, walk-up + 1회 캐싱)로 5곳 단일화, `/healthz` 매 요청 디스크 읽기 해소 |
| build:copy 인라인 스크립트 (v9 A-7) | `package.json:15` | **[DONE — Phase 13-1]** `scripts/build-copy.js`로 추출, 무음 `catch(e){}` 제거(스크립트 부재만 허용) |
| Dockerfile root 실행 + pending에도 healthz 200 (v9 A-8) | `Dockerfile`(USER 없음), `api-server.ts:202-218` | **[DONE — Phase 13-1]** `USER node` 추가, `/healthz` pending→503 + 인증 미들웨어 면제(HEALTHCHECK 401 결함도 발견·수정) |
| 구조화 로깅 (v8 이월) | `src/utils/logger.ts` vs 22개 파일 215곳 `console.*` | **[DONE — Phase 13-8]** Logger를 17개 라이브러리 파일에 배선(모듈 레벨 `const log = new Logger('Ctx')`, 진단성 `console.*` → `log.{info,warn,error}`, 2인자 형태는 `{detail}`로 흡수). stderr 전용 + `data` 내 Error 정규화, `CYNAPX_LOG_LEVEL` 배선. admin CLI 대시보드/REPL 사용자 출력/bootstrap 운영자 배너는 의도적 미변환(stdout 비오염 가드 기존 충족). stdio 비오염 회귀 테스트 추가 |
...
### A-11. PENDING 모드 락 정체성 불일치 — 같은 프로젝트에 Host 둘 가능 **[DONE — Phase 13-3]**
**`src/bootstrap.ts:105-109`**

> **[DONE — Phase 13-3]** `setOnInitialize`가 primary 락 base와 다른 프로젝트 초기화 시 프로젝트 전용 락을 추가 획득(Host ipcPort/nonce 재사용, heartbeat 합류). `LockHeldError` 시 host 서비스 시작을 건너뛰어 기존 Host에 정체성 양보(이중 인덱싱 차단). `tests/bootstrap-failover.test.ts` 신규.
...
### A-12. Terminal 도구 호출 30초 고정 타임아웃 — **[DONE — Phase 13-8]**
**`src/server/ipc-coordinator.ts:196-199`**

> **[DONE — Phase 13-8 커밋 B]** 도구별 타임아웃 테이블(`IPC_TOOL_TIMEOUTS_MS` + `getIpcTimeoutMs()`) 도입 — `initialize_project`/`backfill_history` 30분, `re_tag_project`/`check_consistency` 15분 등, 나머지(빠른 메타데이터 읽기)는 기본 30초. Host가 인증된 idle 연결에 keepalive `{ping:true}` 주기 전송(`unref` 인터벌), Terminal은 id 없는 ping 라인 무시. MCP 2025-11-25 task 워크플로는 후속 Phase 후보로 코멘트만 남김.
...
| # | 위치 | 내용 |
|---|------|------|
| O-1 **[DONE — Phase 13-8]** | `src/graph/graph-engine.ts:510-511` | BFS `queue.shift()` → 인덱스 포인터(`head++`)로 교체 (dfs/reTag 패턴 일치). 거리/순서 회귀 테스트 |
| O-2 **[DONE — Phase 13-4]** | `src/indexer/sync-strategies/full-scan-strategy.ts:18-24` | 풀 스캔 시 파일당 `getLatestCommit` — `getLatestCommitsForFiles()`(`git log --name-only --pretty` 단일 패스, 최신순 first-seen 맵)로 대체. FullScan/Incremental 양쪽 적용 |
| O-3 **[DONE — Phase 13-8]** | `src/server/api-server.ts:206-212` | `/healthz`가 매 요청 package.json 디스크 읽기 — `utils/version.ts` `getVersion()` 캐싱으로 해소(A-10과 동시 처리) |
| O-4 | `src/indexer/typescript-parser.ts:30-42` | **[DONE — Phase 13-8]** 파일마다 `ts.createProgram` + lib 재로딩을 단일 `ts.LanguageService`(+`LanguageServiceHost`, 스크립트 버전 맵)로 교체 — 런 내 incremental Program/lib 캐시 재사용. 출력 동등(회귀 테스트), 벤치 추가 |
| O-5 | `src/graph/graph-engine.ts:168-245` | **(v9 이월)** 클러스터링 전체 메모리 적재 — 현재 규모 무해. verdict: 계속 보류 (단 persistClusters 트랜잭션화는 A-5로 흡수) |
| O-6 | `package.json` | **[DONE — Phase 13-1]** `engines: {node: ">=22"}` 추가 + Docker 베이스 node:22, CI 매트릭스 [22,24] 전환 |
| O-7 **[DONE — Phase 13-8]** | `src/server/interactive-shell.ts:21` | 미등록 `perform_clustering` 표기 제거 — 셸 도구 목록을 `_registry.ts`와 1:1 동기화(테스트로 회귀 방지) |
| O-8 **[DONE — Phase 13-8]** | `src/utils/certificate-generator.ts:32` | 0700 전용 디렉터리(`cynapx-tls-<id>/`) 생성 후 그 안에서 키 작업, 종료 시 디렉터리 통째 정리 — tmp 노출 창 제거 |
| O-9 **[DONE — Phase 13-8]** | `src/utils/audit-logger.ts:84-93` | `readRecent()`를 64KB 청크 후방 tail 읽기로 — 전체 적재 제거(비용이 limit에 비례), 멀티바이트 경계 안전 + 손상 라인 skip |
| O-10 **[DONE — Phase 13-8]** | `src/bootstrap.ts:290-303` | One-shot CLI가 `lifecycle.disposeAll()` + `lockManager.release()` 후 종료(WAL 체크포인트 보장), exitCode 보존 |
| O-11 **[DONE — Phase 13-8]** | `src/utils/file-filter.ts:19-28` | 중첩 .gitignore 지원 — 경로별 조상 디렉터리 .gitignore 지연 로드 + 서브트리 스코프 prefix, 부정 `!` 처리 |
| O-12 **[DONE — Phase 13-8]** | `src/server/tools/search-symbols.ts:15-28` | 컨텍스트는 있으나 전부 `EngineNotReadyError`면 빈 success 대신 `isError` 반환(컨텍스트 0개는 정상 빈 결과 유지) |

---

## 5. 테스트 공백 **[전체 DONE — Phase 13-9 일괄 마감]**

336개 테스트와 통합 스크립트 69 케이스에도 불구하고, 이번 발견의 대부분은 다음 미검증 영역에 있다. **Phase 13-9 종료 시점에 모든 공백이 닫혔다** (525개 테스트, 통합 스크립트 76 케이스 — Docker는 데몬 부재 시 graceful SKIP):

| 공백 | 검증해야 할 시나리오 | 잡았을 결함 | 마감 |
|------|---------------------|------------|------|
| **REST API HTTP 레벨** | 실제 listen + supertest: 인증(401/timing), rate limit(429), `/mcp` 세션 생성·GET 우회·정리, `/healthz` 상태별 코드 | A-2, A-3, v9 A-8 | **[DONE — P13-9]** `tests/api-server-http.test.ts` (supertest, 12케이스). 검증 중 `/healthz`가 `getActiveContext` throw 시 500 누출하던 잠재 결함 발견·수정(503 강등) |
| **Docker 빌드/기동** | `docker build` + `--api` 기동 + healthz smoke | **C-1 전체** | **[DONE — P13-9]** `scripts/docker-smoke.sh`를 통합 스크립트 Phase 27로 배선(Docker 부재 시 SKIP) |
| **IPC 2-프로세스 e2e** | 실제 Host/Terminal 프로세스 분리: 인증 협상(악성 에코 클라이언트 거부), 1MB+ 누적 트래픽, Host kill → failover | C-3, H-8, H-1 | **[DONE — P13-9]** `scripts/ipc-e2e-test.js`(실제 fork된 Host/attacker 프로세스 + 실 TCP) → 통합 스크립트 Phase 26 |
| **purge → 재초기화** | purge_index 후 initialize_project → search_symbols 정상 동작 | H-6 | **[DONE — P13-6/P13-9]** 통합 스크립트 정식 Phase 25 (P13-6에서 편입, P13-9에서 정식 확인) |
| **비-TS 언어 메트릭** | Rust/Go/Java fixture의 CC 기대값 (≠1), 네이티브/JS 경로 동등성 | H-5 | **[DONE — P13-5]** 수정과 동반 작성 |
| **git 이력 재작성** | rebase/force-push 후 syncWithGit가 풀스캔 폴백으로 복구 | H-3 | **[DONE — P13-4]** `tests/git-sync.test.ts` |
| **lock 경합 스트레스** | heartbeat 중 동시 getValidLock 반복(원자성), PID 재사용 시뮬레이션 + 재시도 상한 | H-1, H-2 | **[DONE — P13-9]** `tests/lock-manager.test.ts` "Phase 13-9: lock contention stress"(병렬 ~320 atomic write 무손상, 동시 acquire 단일 승자, PID 재사용 stale→reclaim) |
| **임베딩 프로토콜** | 타임아웃 후 늦은 응답이 다음 배치에 오배달되지 않음 (mock sidecar) | A-7 | **[DONE — P13-8 커밋 A]** 수정과 동반 작성 |
| **크로스 플랫폼** | 케이스 구분 FS에서 해시/경계 검사, python3-only 환경 spawn | C-2, H-7, A-8 | **[DONE — P13-2/P13-7/P13-8]** `resolvePythonCommand`(embedding-queue), `getProjectHash` win32/darwin/posix(phase13-8-commit-b), `isPathInside`(security) — P13-9에서 커버리지 재확인 |
...
- **CVE-2025-7709 — SQLite FTS5 integer overflow (heap OOB write)** **[DONE — Phase 13-7]** (better-sqlite3 → **12.10.0**(SQLite **3.53.1** 로컬 확인), sqlite-vec → **0.1.9**, @modelcontextprotocol/sdk → **1.29.0**. native 바인딩 정상, `sqlite_version() >= 3.50.3` 회귀 테스트 추가): 영향 범위 3.49.1 ≤ v < 3.50.3, 수정 3.50.3. **본 프로젝트의 better-sqlite3 11.10.0이 번들한 SQLite는 3.49.2로 영향 범위 안에 있음을 로컬에서 직접 확인했다**(`sqlite_version()` = 3.49.2). 프로젝트는 FTS5(`fts_symbols`)를 적극 사용하고, `CrossProjectResolver`는 레지스트리에 등록된 **외부 DB 파일을 열어 쿼리**하므로(crafted DB file 공격 벡터) 무시할 수 없는 노출이다. 트리거 조건이 "공격자 제어 쿼리 또는 공격자 제어 DB 파일"이라 위험도는 환경 의존적이지만, 업그레이드가 정석. 출처: [openwall oss-security](https://www.openwall.com/lists/oss-security/2025/09/06/2), [GitHub advisory GHSA-v2c8-vqqp-hv3g](https://github.com/google/security-research/security/advisories/GHSA-v2c8-vqqp-hv3g)
