...
## 2. Phase 27-1: escapeXml/escapeDot escape 동작 게이트 (M-1 v24) [예정]
...
## 3. Phase 27-2: better-sqlite3 lockfile 12.10.0 → 12.11.1 정렬 (M-2 v24) [예정]
...
| 27-1 [예정] | M-1 v24: `tests/_utils.test.ts`에 `escapeXml`/`escapeDot` escape 동작 게이트(의존 0 순수 함수, 테스트-only) — 4개 엔티티 치환·`&`-우선 순서(이중-이스케이프 회피)·전역 치환·무변경·escapeDot `\`/`"` 치환 (신규 ~4-6 케이스, vitest 642→~646-648) | 1 | 매우 낮음 |
| 27-2 [예정] | M-2 v24: better-sqlite3 lockfile 12.10.0→12.11.1 정렬(lockfile-only, `package.json` 무변경) + 재빌드 + 642 그린·audit 0/0 재확인 | 1 (27-1과 합본 가능) | 매우 낮음 |
