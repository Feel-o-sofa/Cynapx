...
## 2. Phase 22-1: architecture-engine O(E)→O(1) Map + 미커버 circular-dependency 분기 회귀 게이트 (M-1 v19) — 작고·테스트 동반·저위험 [DONE]
...
| 22-1 **[DONE]** | M-1 v19: architecture-engine.ts:179 `edges.find` O(E) → `Map` O(1) + 미커버 circular-dependency 분기 회귀 테스트 신규 + diagnostic-v19 [DONE] — **vitest 594 그린**(593+신규 1), tsc 그린, audit 0/0(불변) | 1 | 매우 낮음 (국소 ~5줄·시그니처 무변경·동작 동일 + 회귀 테스트가 동작 동일성 못 박음) |
