...
## 2. Phase 20-1 [DONE]: FileWatcher 대용량-배치 + 복구 상태머신 vitest 테스트 추가 (M-1 v17) — 테스트-only·무위험
...
| 20-1 **[DONE]** | M-1 v17: FileWatcher 대용량-배치/복구 vitest 테스트 추가(대용량 트리거→syncWithGit 우선 + 재시도/FATAL 강등/git-sync 락인) + diagnostic-v17 [DONE] — **593/593 그린**(588 + 신규 5 케이스), tsc 그린, audit 0 vulns | 1 | 매우 낮음 (테스트-only, 프로덕션 코드 무변경) |
