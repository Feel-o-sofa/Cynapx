...
*   **리소스**: `graph://summary` (전체 통계) 제공 중.
*   **프롬프트**: `explain-impact`, `check-health` 워크플로우 지원 중.
...
*   **Task 17: Capabilities 확장 (Resources/Prompts)**:
    *   **Resource (`graph://ledger`)**: 전역 호출 장부 상태를 노출하여 에이전트가 인덱스 신뢰도를 스스로 판단하게 함. (완료)
    *   **Resource (`graph://hotspots`)**: `get_hotspots` 도구의 결과를 리소스 형태로 상시 노출. (대기)
    *   **Prompt (`/refactor-safety`)**: 특정 심볼 수정 전 영향 범위와 정합성을 동시 체크하는 고도화된 워크플로우. (대기)
...
**Context**: Basic tools are stable. Moving towards "Self-Awareness" (via Ledger resource) and "Rich Feedback".
